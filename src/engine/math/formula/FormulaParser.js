...
/**
 * Parses infix arithmetic expressions given a token map that says what each word and operator means.
 *
 * A token map entry is a value (a constant), the string "(" or ")", or an operation
 * `{unary_action, binary_action, priority}`. Adjacent values multiply implicitly.
 */
class FormulaParser {
  /**
   ...
   * @template T
   */
  static parse(text, tokenMap) {
    let tokens = FormulaParser._tokenize(text).map((e) =>
      FormulaParser._translateToken(e, tokenMap),
    );
    ...
    let apply = (op) => {
      ...
      if (vals.length < 2) {
        throw new DetailedError("Bad expression: operated on nothing", {
          text,
        });
      }
      let b = vals.pop();
    ...
    let feedOp = (couldBeBinary, token) => {
      ...
      if (couldBeBinary && token.binary_action !== undefined) {
        ...
        throw new DetailedError("Bad expression: binary op in bad spot", {
          text,
        });
      }
    };
  ...
  /**
   * Joins "1", "e", "-", "5" style token runs back into the single float token "1e-5".
