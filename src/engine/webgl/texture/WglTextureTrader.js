...
class WglTextureTrader {
   ...
   *
   * If rendering fails, the new texture goes back to the pool and the trader keeps the old one.
   *
  ...
  shadeAndTrade(shaderFunc, newTexture = undefined) {
    ...
    this._dontDeallocFlag = false;
    if (deallocSrc) {
      src.deallocByDepositingInPool("WglTextureTrader shadeAndTrade");
    }
  }
  ...
  shadeHalveAndTrade(reducingShaderFunc) {
    this.shadeAndTrade(
      reducingShaderFunc,
      WglTexturePool.take(
        Math.max(0, this.currentTexture.sizePower() - 1),
        this.currentTexture.pixelType,
      ),
    );
  }
  ...
  shadeQuarterAndTrade(reducingShaderFunc) {
    this.shadeAndTrade(
      reducingShaderFunc,
      WglTexturePool.take(
        Math.max(0, this.currentTexture.sizePower() - 2),
        this.currentTexture.pixelType,
      ),
    );
  }
