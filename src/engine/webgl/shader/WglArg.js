...
WglArg.INPUT_ACTION_MAP = new Map([
  ...
  [
    WglArg.MAT4_ARRAY_TYPE,
    (ctx, loc, buf) => ctx.gl.uniformMatrix4fv(loc, false, buf),
  ],
  [
    WglArg.VEC4_TYPE,
    (ctx, loc, [r, g, b, a]) => ctx.gl.uniform4f(loc, r, g, b, a),
  ],
  [
    WglArg.MAT4_TYPE,
    (ctx, loc, val) => ctx.gl.uniformMatrix4fv(loc, false, val),
  ],
  [
    ...
    (ctx, loc, texture, coop) => {
      ...
      if (
        texture.width > ctx.maxTextureSize ||
        texture.height > ctx.maxTextureSize
      ) {
        throw new Error(
          `Uniform texture argument is ${texture.width}x${texture.height}, but max ` +
