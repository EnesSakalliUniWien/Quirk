...
function NOT_RENDERER(args) {
  ...
  circle(args.painter, drawArea.center(), drawArea.w / 2, {
    stroke: { color: style.fill, width: 1 },
  });
  ...
  if (hasSingleWireControl || !hasDoubleWireControl) {
    strokePath(
      args.painter,
      [drawArea.topCenter(), drawArea.bottomCenter()],
      style.text,
      1,
    );
  }
  if (hasDoubleWireControl) {
    strokePath(
      args.painter,
      [
        drawArea.topCenter().offsetBy(-1, 0),
        drawArea.bottomCenter().offsetBy(-1, 0),
      ],
      style.text,
      1,
    );
  ...
  if (isMeasured) {
    strokePath(
      args.painter,
      [
        drawArea.centerLeft().offsetBy(0, -1),
        drawArea.centerRight().offsetBy(0, -1),
      ],
      style.text,
      1,
    );
    ...
    strokePath(
      args.painter,
      [drawArea.centerLeft(), drawArea.centerRight()],
      style.text,
      1,
    );
  }
...
HalfTurnGates.H = new GateBuilder()
  ...
  .setKnownEffectToMatrix(QubitMatrix.HADAMARD).gate;
