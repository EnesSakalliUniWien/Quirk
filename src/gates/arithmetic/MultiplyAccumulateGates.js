...
import { SECTIONED_RENDERER_MAKER } from "../../draw/gate/GateRenderers.js";
import {
  ketArgs,
  ketShaderPermute,
  ...
  (span, builder) =>
    builder
      ...
      .setKnownEffectToParametrizedPermutation(
        (t, a, b) => (t + a * b) & ((1 << span) - 1),
      ),
);

MultiplyAccumulateGates.MultiplySubtractInputsFamily = Gate.buildFamily(
  1,
  16,
  (span, builder) =>
    builder
...
MultiplyAccumulateGates.SquareAddInputFamily = Gate.buildFamily(
  1,
  16,
  (span, builder) =>
    builder
      ...
      .setActualEffectToUpdateFunc((ctx) =>
        MultiplyAccumulateGates.MultiplyAddInputsFamily.ofSize(
          span,
        ).customOperation(ctx.withInputSetToOtherInput("B", "A")),
      )
      .setKnownEffectToParametrizedPermutation(
        (t, a) => (t + a * a) & ((1 << span) - 1),
      ),
...
MultiplyAccumulateGates.SquareSubtractInputFamily = Gate.buildFamily(
  1,
  16,
  (span, builder) =>
    builder
      ...
      .setRequiredContextKeys("Input Range A")
      .setActualEffectToUpdateFunc((ctx) =>
        MultiplyAccumulateGates.MultiplySubtractInputsFamily.ofSize(
          span,
        ).customOperation(ctx.withInputSetToOtherInput("B", "A")),
      )
      .setKnownEffectToParametrizedPermutation(
