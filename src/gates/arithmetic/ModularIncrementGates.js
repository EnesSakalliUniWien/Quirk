...
ModularIncrementGates.IncrementModRFamily = Gate.buildFamily(
  1,
  16,
  (span, builder) =>
    builder
      ...
      .setActualEffectToShaderProvider((ctx) =>
        MODULAR_INCREMENT_SHADER.withArgs(
          ...ketArgs(ctx, span, ["R"]),
          WglArg.float("amount", +1),
        ),
      )
      .setKnownEffectToParametrizedPermutation((t, a) =>
        t < a ? (t + 1) % a : t,
      ),
);

ModularIncrementGates.DecrementModRFamily = Gate.buildFamily(
  1,
  16,
  (span, builder) =>
    builder
