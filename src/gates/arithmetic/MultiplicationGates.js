...
MultiplicationGates.TimesAFamily = Gate.buildFamily(1, 16, (span, builder) =>
  builder
    ...
    .setActualEffectToShaderProvider((ctx) =>
      MULTIPLICATION_SHADER.withArgs(...ketArgs(ctx, span, ["A"])),
    )
    .setKnownEffectToParametrizedPermutation((x, a) =>
      modularMultiply(x, a, 1 << span),
    ),
);

MultiplicationGates.TimesAInverseFamily = Gate.buildFamily(
  1,
  16,
  (span, builder) =>
    builder
      ...
      .setRequiredContextKeys("Input Range A")
      .setKnownEffectToParametrizedPermutation((x, a) =>
        modularUnmultiply(x, a, 1 << span),
      )
      .setActualEffectToShaderProvider((ctx) =>
        INVERSE_MULTIPLICATION_SHADER.withArgs(...ketArgs(ctx, span, ["A"])),
      ),
);
