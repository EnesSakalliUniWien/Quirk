...
export function GatePreview({gate}) {
    const circuit = useMemo(() => new CircuitDefinition(gate.height,
        [new GateColumn(Array.from({length:gate.height}, (_, row) => row === 0 ? gate : undefined))]), [gate]);
    return <section className="gate-construction-preview"><h3>Circuit appearance</h3>
