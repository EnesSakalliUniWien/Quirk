...
class Util {
  ...
  static bin(number, fixedLen) {
    return ("0".repeat(fixedLen) + number.toString(2))
      .slice(-fixedLen)
      .split("")
      .join("");
  }
   ...
   * @template K, V
   */
  static reverseGroupMap(
    groupMap,
    includeGroupsForOriginalKeysEvenIfEmpty = false,
  ) {
   ...
   * @returns {!int}
   */
  static binarySearchForTransitionFromTrueToFalse(
    max,
    argIsBeforeTransitionFunc,
  ) {
    let min = 0;
    while (max > min) {
  ...
  static breakLine(text, maxWidth, measureWidth) {
    ...
    while (p < text.length) {
      ...
      let maxKeepLength =
        Util.binarySearchForTransitionFromTrueToFalse(
          text.length - p + 1,
          (i) => measureWidth(text.slice(p, p + i)) <= maxWidth,
        ) - 1;
      maxKeepLength = Math.max(1, maxKeepLength);
      const maxChunk = text.slice(p, p + maxKeepLength);

      const hitBoundary =
        p + maxKeepLength === text.length ||
        text.slice(p + maxKeepLength, p + maxKeepLength + 1).match(/\s/) !==
          null;
      if (!hitBoundary) {
  ...
  static objectifyArrayFunc(func) {
    return (arg) =>
      Util.recomposedObjectValues(arg, func(Util.decomposeObjectValues(arg)));
  }
