 ...
 *
 * Built from the five facts that determine every rect, so rendering and hit testing can share one
 * answer to "where is that?" without reaching into the circuit widget's private state. Immutable;
 * geometry/CircuitLayout.js derives and caches these for immutable display snapshots.
 */
class CircuitGeometry {
    ...
    clampedCircuitColCount() {
        return Math.max(
            ...
            Layout.MIN_COL_COUNT + (this.compressedColumnIndex !== undefined ? 1 : 0));
    }
