...
class DisplayedInspector {
  ...
  paint(painter, stats, playheadStep = undefined) {
    ...
    painter.group("circuit", (view) =>
      this.displayedCircuit.paint(
        view,
        this.hand,
        stats,
        false,
  ...
  withJustEnoughWires(hand, extraWires) {
    return this.withDisplayedCircuit(
      this.displayedCircuit.withJustEnoughWires(hand, extraWires),
    );
  }
  ...
  afterDropping() {
    return this.withDisplayedCircuit(
      this.displayedCircuit.afterDropping(this.hand),
  ...
  withCircuitDefinition(newCircuitDefinition) {
    return new DisplayedInspector(
      this.drawArea,
      DisplayedCircuit.empty(Layout.CIRCUIT_TOP_MARGIN).withCircuit(
  ...
  desiredHeight() {
    return (
      this.displayedCircuit.desiredHeight() + 2 * Layout.CIRCUIT_TOP_MARGIN
    );
  }
