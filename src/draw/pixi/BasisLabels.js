...
export class BasisLabels {
    constructor(size, draw) { this.size = size; this.draw = draw; this.version = 0; this.id = nextId++; }
    clear() { this.version++; }
    paint(x, y, view, key) {
        view.group('basis-' + this.id + '-' + view.order, placement => {
            placement.position.set(x, y);
            const content = placement.use(DisplayView, 'labels');
            const version = [this.version, textLayoutVersion, view.pixelRatio, key].join(':');
