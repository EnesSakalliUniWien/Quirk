...
export class RenderSurface {
    ...
    constructor(canvas) {
        ...
        this.ready = this.app.init({canvas, width: Math.max(1, canvas.width), height: Math.max(1, canvas.height),
            preference: 'webgl', autoStart: false, sharedTicker: false, antialias: true,
            backgroundAlpha: 0, preserveDrawingBuffer: true}).then(() => {
    ...
    beginFrame(rng, pixelRatio = 1) {
        ...
        if (!this.pending) {
            this.pending = true;
            queueMicrotask(() => this.render().catch(error => reportBlockingIssue(`Rendering failed: ${error.message}`)));
        }
        return this.view;
    }
    async render() {
        ...
        this.view.finish();
        this.app.renderer.resize(this.width || this.canvas.width, this.height || this.canvas.height, 1);
    ...
    async destroy() {
        ...
        await this.ready;
        if (this.app.renderer) { this.app.destroy(false, {children: true}); }
    }
}
