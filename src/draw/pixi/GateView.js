...
export class GateView extends DisplayView {
    update(args, drawer) {
        ...
        const inputs = [gate, drawer, rect.x, rect.y, rect.w, rect.h, args.isHighlighted,
            args.isResizeShowing, args.isResizeHighlighted, this.pixelRatio, textLayoutVersion];
        // The default static symbol does not read simulation data, focus points, or randomness.
        const staticSymbol = !gate.customDrawer && gate.stableDuration() === Infinity;
        if (staticSymbol && this.inputs && inputs.every((value, i) => Object.is(value, this.inputs[i]))) {
