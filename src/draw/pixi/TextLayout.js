...
export function drawText(view, text, {x = 0, y = 0, fill = CanvasTheme.text.primary, font = DEFAULT_FONT,
    ...
    const node = view.use(LabelView);
    node.update(text, font, fill, view.pixelRatio, stroke, textLayoutVersion);
    const {ascent, descent} = measureText(text, font).fontProperties;
    node.anchor.set(align === 'center' ? 0.5 : align === 'right' || align === 'end' ? 1 : 0,
