...
import {traceProbabilitySquare, traceAmplitudeProbabilityCircle, traceAmplitudeLogarithmCircle, LOG_RING_WIDTH, traceAmplitudePhaseDirection, PHASE_HAND_WIDTH} from './ComplexCellGeometry.js';
...
export function paintDensityMatrix(painter,
        ...
        if (!hasNaN) {
            drawPath(painter, traceDiagonalWith(traceProbabilitySquare), [{fill: fillColor}, {stroke: {color: CanvasTheme.probability.outline, width: 0.5}}]);

            drawPath(painter, traceCouplingsWith(traceAmplitudeProbabilityCircle), [{fill: fillColor}, {stroke: {color: CanvasTheme.probability.outline, width: 0.5}}]);

            drawPath(painter, traceCouplingsWith(traceAmplitudeLogarithmCircle), [{stroke: {color: CanvasTheme.stroke.faint, width: LOG_RING_WIDTH}}]);

            drawPath(painter, traceCouplingsWith(traceAmplitudePhaseDirection), [{stroke: {color: CanvasTheme.amplitude.phaseHalo, width: 3}}, {stroke: {color: CanvasTheme.text.primary, width: PHASE_HAND_WIDTH}}]);
        }
        ...
        paintMatrixTooltip(painter, matrix, drawArea, focusPoints,
            (c, r) => c === r ?
