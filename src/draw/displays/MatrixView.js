...
import {
    traceAmplitudeProbabilitySquare, traceAmplitudeProbabilityCircle,
    traceAmplitudeLogarithmCircle, traceAmplitudePhaseDirection, PHASE_HAND_WIDTH, LOG_RING_WIDTH
...
class MatrixGraphics extends Graphics {
    ...
    set picture(picture) {
        ...
        if (same(this.previous?.values, values) && same(this.previous?.buf, buf) && same(this.previous?.colors, colors)) return;
        this.previous = {values, buf: buf.slice(), colors};
        this.clear();
        const [x, y, diam, numCols, numRows, hasNaN, amplitudeCircleFillColor,
            amplitudeCircleStrokeColor, amplitudeProbabilityFillColor, backColor,
            logCircleStrokeColor, showLogCircles] = values;
        ...
        const cells = trace => geometry => {
            for (let row = 0; row < numRows; row++) {
                for (let col = 0; col < numCols; col++) {
                    ...
                    trace(geometry, buf[k], buf[k + 1], x + diam * col, y + diam * row, diam);
                }
            }
        };
        if (!hasNaN) {
            if (amplitudeProbabilityFillColor !== undefined) {
                path(cells(traceAmplitudeProbabilitySquare), [{fill: amplitudeProbabilityFillColor},
                    {stroke: {color: CanvasTheme.stroke.grid, width: 0.5}}]);
            }
            if (amplitudeCircleFillColor !== undefined) {
                path(cells(traceAmplitudeProbabilityCircle), [{fill: amplitudeCircleFillColor},
                    {stroke: {color: amplitudeCircleStrokeColor, width: 0.5}}]);
                if (showLogCircles) {
                    path(cells(traceAmplitudeLogarithmCircle), [{stroke: {color: logCircleStrokeColor, width: LOG_RING_WIDTH}}]);
                }
            }
        }
        path(trace => PathGeometry.grid(trace, x, y, diam * numCols, diam * numRows, numCols, numRows),
        ...
        for (const [color, indices] of cellsByColor) {
            path(trace => {
                for (const index of indices) {
                    traceAmplitudePhaseDirection(trace, buf[2 * index], buf[2 * index + 1],
                        x + diam * (index % numCols), y + diam * Math.floor(index / numCols), diam);
                }
            }, [{stroke: {color: CanvasTheme.amplitude.phaseHalo, width: 3}},
...
export function paintMatrix(painter, matrix, drawArea, {
    ...
    const numCols = matrix.width(), numRows = matrix.height();
    ...
    if (!hasNaN && showPhase) {
        for (let k = 0; k < buf.length; k += 2) {
            colors.push(buf[k] === 0 && buf[k + 1] === 0 ? undefined :
                phaseColorForDegrees(Math.atan2(buf[k + 1], buf[k]) * 180 / Math.PI));
        }
    }
    painter.add('pixiMatrixGraphics', {picture: {
        values: [drawArea.x, drawArea.y, diam, numCols, numRows, hasNaN,
            amplitudeCircleFillColor, amplitudeCircleStrokeColor, amplitudeProbabilityFillColor,
            backColor, logCircleStrokeColor, showLogCircles], buf, colors
    }});
    if (hasNaN) {
        fitText(painter, 'NaN', {
            x: drawArea.x + diam * numCols / 2,
            y: drawArea.y + diam * numRows / 2,
            align: 'center', baseline: 'middle', fill: CanvasTheme.error.text,
            font: {fontSize: 16, fontFamily: Typography.DEFAULT_FONT_FAMILY},
            width: diam * numCols, height: diam * numRows
        });
    }
}
