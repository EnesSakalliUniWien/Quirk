 ...
 */
function projectionGeometry(vec, plane, reading = blochReading(vec)) {
 ...
 * @param {!("meridian"|"equator")} plane
 * @param {!{focusAxis?: !string, layers?: !Object, reading?: !import('../../../engine/math/bloch.js').BlochReading}=} options
 */
function paintBlochProjection(view, width, height, vec, plane, {focusAxis, layers = {trig: true}, reading} = {}) {
    rectangle(view, new Rect(0, 0, width, height), {fill: CanvasTheme.surface.background});
    const geometry = projectionGeometry(vec ?? {x: 0, y: 0, z: 0}, plane, reading);
