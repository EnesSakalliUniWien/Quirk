...
suite.test("Register labels fit, ket hits stay within their control, and insertion clears the gutter", () => {
    const displayed = CircuitViewState.empty(0).withCircuit(new CircuitDefinition(Simulation.MAX_WIRE_COUNT, []));
    ...
    for (let row = 0; row < Simulation.MAX_WIRE_COUNT; row++) {
        ...
        assertThat(findWireWithInitialStateAreaContaining(displayed.geometry(), ket.center())).isEqualTo(row);
        assertThat(findWireWithInitialStateAreaContaining(displayed.geometry(), index.center())).isEqualTo(undefined);
        assertThat(findWireWithInitialStateAreaContaining(displayed.geometry(), new Point(ket.center().x, ket.bottom() + 1))).isEqualTo(undefined);
        assertThat(displayed.tryClick(PointerInteractionState.EMPTY.withPos(ket.center())).circuitDefinition.customInitialValues.get(row)).isEqualTo('1');
    }
...
suite.test("Bloch bounds fit their row and column and include the enlarged click area", () => {
    ...
    const displayed = CircuitViewState.empty(0).withCircuit(definition);
    const g = displayed.geometry();
    ...
    const found = findGateOverlappingPos(displayed.geometry(), edge);
