...
import {blochReading} from '../../../../src/engine/math/bloch.js';

const suite = new Suite('BlochProjections');

suite.test('shared readings preserve sphere and projection pixels across state changes', async () => {
    ...
    try {
        for (const vec of [{x: 0.5, y: 0.5, z: Math.SQRT1_2}, {x: 0, y: 0, z: -1},
            {x: 0, y: 0, z: 0}, undefined]) {
            const reading = vec === undefined ? undefined : blochReading(vec);
            for (const draw of [
                options => drawBlochScene(canvas, vec, 0.3, 0.2, options),
                options => drawBlochProjection(canvas, vec, 'meridian', options),
                options => drawBlochProjection(canvas, vec, 'equator', options)
            ]) {
                draw({});
                ...
                draw({reading});
                await surface.render();
                assertThat(canvas.toDataURL()).isEqualTo(expected);
            }
        }
    } finally {
