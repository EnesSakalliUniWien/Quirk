...
import {fitLine} from '../../../src/draw/text/TextLayout.js';
import {drawingArea} from '../../../src/draw/scene/DisplayView.js';
...
suite.test('retains gate objects across frames and disposes removed gates', async () => {
    ...
    const container = gate.native;
    view.begin(); view.position.set(30, 0);
    ...
    assertThat(updated.native === container).isEqualTo(true);
    assertThat(updated.children[0] === node).isEqualTo(true);
    assertThat([...(await scenePixels(canvas, 10, 10, 1, 1)).data]).isEqualTo([0, 0, 0, 0]);
    view.begin(); await view.commit();
    assertThat(container.destroyed).isEqualTo(true);
    assertThat(view.children.length).isEqualTo(0);
});
...
suite.test('unmounting a rendered surface destroys nested graphics and text', async () => {
    const canvas = document.createElement('canvas');
    const surface = RenderSurface.forCanvas(canvas);
    const view = surface.beginFrame();
    let shape;
    let text;
    const group = view.group('nested', child => {
        shape = rectangle(child, new Rect(0, 0, 10, 10), {fill: 'blue'});
        text = drawText(child, 'label');
    });
...
suite.test('unchanged fixed shapes retain geometry and changed colours update it', async () => {
    ...
    const first = rectangle(view, bounds, {fill: 'red'});
    await view.commit();
    const shape = first.native;
    const instruction = shape.context.instructions[0];
    view.begin(); const second = rectangle(view, bounds, {fill: 'red'}); await view.commit();
    assertThat(second.native === shape).isEqualTo(true);
    assertThat(shape.context.instructions[0] === instruction).isEqualTo(true);
    view.begin(); rectangle(view, bounds, {fill: 'blue'}); await view.commit();
...
suite.test('native parent transforms preserve nested nonuniform scale and rotation', async () => {
    const view = new DisplayView(document.createElement('canvas'));
    ...
    await view.commit();
    const point = child.native.toGlobal({x: 4, y: 5});
    assertThat(point.x).isApproximatelyEqualTo(0);
    assertThat(point.y).isApproximatelyEqualTo(32);
    const local = child.native.toLocal(point);
    assertThat(local.x).isApproximatelyEqualTo(4);
    assertThat(local.y).isApproximatelyEqualTo(5);
});

suite.test('basis labels update only after their inputs change and keep placement independent', async () => {
    const view = new DisplayView(document.createElement('canvas'));
    let builds = 0;
...
suite.test('reused labels clear an old stroke when the next appearance has none', async () => {
    const view = new DisplayView(document.createElement('canvas'));
    const first = drawText(view, 'click', {stroke: {color: 'red', width: 3}}); await view.commit();
...
suite.test('a frame sits on its edge and the highlight ring just outside it, on whole pixels', async () => {
    const view = new DisplayView(document.createElement('canvas'));
