...
suite.testUsingWebGL('formulaic_matrices', () => {
    ...
    assertThat(Gates.ParametrizedRotationGates.FormulaicRotationRz.withParam('t pi').knownMatrixAt(0.5)
    ...
    assertThat(Gates.ParametrizedRotationGates.FormulaicRotationRx.withParam('t pi').knownMatrixAt(0.25)
            ...
            1, Complex.I.neg(),
            Complex.I.neg(), 1).times(Math.sqrt(0.5)));
    assertThat(Gates.ParametrizedRotationGates.FormulaicRotationRx.withParam('t pi').knownMatrixAt(0.5)
    ...
    assertThat(Gates.ParametrizedRotationGates.FormulaicRotationRy.withParam('t pi').knownMatrixAt(0.25)
            ...
            1, -1,
            1, 1).times(Math.sqrt(0.5)));
    assertThat(Gates.ParametrizedRotationGates.FormulaicRotationRy.withParam('t pi').knownMatrixAt(0.5)
        ).isApproximatelyEqualTo(Matrix.square(0, -1, 1, 0));
});
...
suite.test('formulaic_t_matches_builtin_powering_gates', () => {
    for (let time of [0, 1/8, 1/4, 1/2, 6/7]) {
        ...
        assertThat(Gates.ParametrizedRotationGates.FormulaicRotationY.withParam('t').knownMatrixAt(time)).
            withInfo({time}).
            isApproximatelyEqualTo(Gates.Powering.YForward.knownMatrixAt(time), 0.0001);
        assertThat(Gates.ParametrizedRotationGates.FormulaicRotationZ.withParam('t').knownMatrixAt(time)).
            withInfo({time}).
            isApproximatelyEqualTo(Gates.Powering.ZForward.knownMatrixAt(time), 0.0001);
    }
});
...
suite.test('invalid_formula_is_not_time_dependent', () => {
    ...
    assertThat(g.withParam('atna(t)').stableDuration()).isEqualTo(Infinity);
});
